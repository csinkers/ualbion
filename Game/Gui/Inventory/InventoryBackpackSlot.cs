...
namespace UAlbion.Game.Gui.Inventory
{
    sealed class InventoryBackpackSlot : InventorySlot
    {
        ...
        public InventoryBackpackSlot(PartyCharacterId activeCharacter, int slotNumber)
            : base(AssetType.PartyMember, (int)activeCharacter, BackpackHandlers)
        {
    ...
    sealed class InventoryChestSlot : InventorySlot
    {
        protected override ButtonFrame Frame { get; }
        readonly UiSpriteElement<ItemSpriteId> _sprite;
        readonly int _slotNumber;
        ...
        int _frameNumber;
        protected override ItemSlotId SlotId => (ItemSlotId)((int)ItemSlotId.Slot0 + _slotNumber);
        static readonly HandlerSet ChestHandlers = new HandlerSet(SlotHandlers,
            H<InventoryChestSlot, InventoryChangedEvent>((x, e) => x._version++),
            H<InventoryChestSlot, SlowClockEvent>((x, e) => x._frameNumber += e.Delta)
        );
        ...
        public InventoryChestSlot(ChestId chest, int slotNumber)
            : base(AssetType.ChestData, (int)chest, ChestHandlers)
        {
            _slotNumber = slotNumber;
            _sprite = new UiSpriteElement<ItemSpriteId>(0) { SubId = (int)ItemSpriteId.Nothing };
