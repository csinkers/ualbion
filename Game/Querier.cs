...
{
    public class Querier : Component, IQuerier
    {
        ...
        public bool Query(EventChainContext context, IQueryEvent query)
        {
            ...
            switch (query, query.QueryType)
            {
                ...
                case (QueryItemEvent itemQuery, QueryType.InventoryHasItem): return Compare(itemQuery.Operation, game.Party.GetItemCount(itemQuery.ItemId), itemQuery.Immediate);
                case (QueryItemEvent itemQuery, QueryType.UsedItemId): return context.UsedItem == itemQuery.ItemId;
        ...
        bool Compare(QueryOperation operation, int value, int immediate) =>
            operation switch
            {
                ...
                QueryOperation.Unk4 => (value == immediate),
