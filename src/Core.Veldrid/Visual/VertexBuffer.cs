...
{
    public class VertexBuffer : VeldridComponent
    {
        readonly string _name;
        readonly uint _size;
        DeviceBuffer _buffer;

        public VertexBuffer(string name, uint size)
        {
            _name = name;
            _size = size;
        }
        ...
        public override void CreateDeviceObjects(VeldridRendererContext context)
        {
            ...
            _buffer = context.GraphicsDevice.ResourceFactory.CreateBuffer(new BufferDescription(_size, BufferUsage.VertexBuffer));
            _buffer.Name = _name;
        }
