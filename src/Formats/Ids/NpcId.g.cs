...
public readonly struct NpcId : IEquatable<NpcId>, IEquatable<AssetId>, IComparable, IAssetId
{
    ...
    {
        if (mapping == null) throw new ArgumentNullException(nameof(mapping));
        var (enumType, enumValue) = mapping.IdToEnum(new NpcId(AssetType.NpcSheet, disk));
    ...
    public static implicit operator SheetId(NpcId id) => SheetId.FromUInt32(id._value);
    public static explicit operator NpcId(SheetId id) => new NpcId(id.ToUInt32());
    ...
    public static implicit operator NpcId(UAlbion.Base.NpcSheet id) => NpcId.From(id);
