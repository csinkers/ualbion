...
public readonly struct PortraitId : IEquatable<PortraitId>, IEquatable<AssetId>, IComparable, IAssetId
{
    ...
    public PortraitId(int id)
    {
#if DEBUG
        if (id < 0 || id > 0xffffff)
            throw new ArgumentOutOfRangeException($"Tried to construct a PortraitId with out of range id {id}");
#endif
        _value = (uint)AssetType.Portrait << 24 | (uint)id;
    }
    ...
    public static AssetType[] ValidTypes = { AssetType.Portrait };
    public static PortraitId Parse(string s) => AssetMapping.Global.Parse(s, ValidTypes);
