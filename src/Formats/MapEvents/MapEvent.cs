...
public abstract class MapEvent : Event, IMapEvent
{
    ...
    public static EventNode SerdesNode(ushort id, EventNode node, ISerdes s, AssetMapping mapping, MapType mapType)
    {
        ...
        if (@event is IBranchingEvent be)
        {
            ...
            ushort rawEventId = branch.NextIfFalse?.Id ?? 0xffff;
            rawEventId = s.UInt16(nameof(branch.NextIfFalse), rawEventId);
            ushort? falseEventId = rawEventId == 0xffff ? null : rawEventId;
        ...
        ushort rawNextEventId = node.Next?.Id ?? 0xffff;
        rawNextEventId = s.UInt16(nameof(node.Next), rawNextEventId);
        ushort? nextEventId = rawNextEventId == 0xffff ? null : rawNextEventId;
    ...
    public static IMapEvent SerdesEvent(IMapEvent e, ISerdes s, AssetMapping mapping, MapType mapType)
    {
        ArgumentNullException.ThrowIfNull(s);
        var initialPosition = s.Offset;
        s.Begin();
