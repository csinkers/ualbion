...
namespace UAlbion.Game.Veldrid.Assets
{
    public class NpcTilesetLoader : Component, IAssetLoader
    {
        ...
        public object Serdes(object existing, AssetInfo info, AssetMapping mapping, ISerializer s)
        {
            ...
            if (s.IsWriting())
            {
                ...
                foreach (var id in assetIds)
                {
                    ...
                    if (palId == 0)
                        spriteInfo.Set(AssetProperty.PaletteId, assets.GetAssetInfo(id).Get(AssetProperty.PaletteId, 0));

                    var path = spriteInfo.BuildFilename(graphicsPattern, 9); // 9 = First frame facing west for both large and small
        ...
        void WriteNpcSprite(string path, ITexture sprite, AssetInfo info, IFileSystem disk, AssetMapping mapping)
        {
            ...
            int i = 0;
            foreach (var chunk in PackedChunks.Unpack(s))
            {
                if (i == 9)
                {
