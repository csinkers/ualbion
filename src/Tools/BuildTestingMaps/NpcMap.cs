            ...
            MajMin(cellArrayW, cellArrayH, (majI, majJ) =>
            {
                ...
                MajMin(cellW, cellH, (i, j) =>
                {
                    if (i == 0 || j == 0 || i == (cellW - 1) || j == (cellH - 1))
                        m.Tiles[Pos(x0 + i, y0 + j)].Underlay = tileset1.SolidOffset; // Draw cell walls
                });

                m.Tiles[Pos(x0 + 1, y0 + 1)].Underlay = (ushort)(tileset1.UnderlayOffset + cellIndex); // Add marker
                if (cellIndex < 250)
                    m.AddZone((byte)(x0 + 1), (byte)(y0 + 1), TriggerTypes.Manipulate, (ushort)cellIndex);

                if (cellIndex != 0x83 && cellIndex != 0x84)
                    return;

                var waypoints = BuildPatrolPath(x0+3, y0+6);
                m.Npcs.Add(new MapNpc {
